...
class DailyWork:
    def go_work(self):
        ...
        if not self.wait_click_feature(feature=fL.work_enter, raise_if_not_found=False, click_after_delay=0.5):
            ...
            return False
        if self.click_ok(time_out=8, click_after_delay=0.5, blind_point=(0.504, 0.042)):
            self.click_ok(settle_time=2, time_out=4, click_after_delay=0.5, blind_point=(0.504, 0.042))
        ...
        while self.wait_click_feature(feature=fL.work_select_enter, time_out=4, raise_if_not_found=False, click_after_delay=0.5, box=self.box_of_screen(0.226, 0.468, 0.769, 0.527)):
            ...
            for i in range(2):
                ...
                if i==0:
                    self.click_ok(time_out=2, click_after_delay=0.5)
            if not self.click_ok(time_out=4, click_after_delay=0.5):
